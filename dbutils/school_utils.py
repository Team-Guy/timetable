...
def get_faculty_activities(*, prof=None, subject=None, type=None, spec=None):
    if not prof and not subject and not type and not spec:#0
        ...
        activities = []
        for group in Specialization.groups[spec]:
            activities.extend([FacultyActivity(activity) for activity in SchoolActivity.objects.filter(professor=prof,group=group)])
        return activities
        ...
        return [FacultyActivity(activity) for activity in SchoolActivity.objects.filter(professor=prof,title=subject, type=type)]
    elif not type:#1 2 4
        activities = []
        for group in Specialization.groups[spec]:
            activities.extend(
                [FacultyActivity(activity) for activity in SchoolActivity.objects.filter(professor=prof,title=subject, group=group)])
        return activities
    elif not subject:#1 3 4
        activities = []
        for group in Specialization.groups[spec]:
            activities.extend(
                [FacultyActivity(activity) for activity in SchoolActivity.objects.filter(professor=prof,type=type, group=group)])
        return activities
    elif not prof:#2 3 4
        activities = []
        for group in Specialization.groups[spec]:
            activities.extend(
                [FacultyActivity(activity) for activity in SchoolActivity.objects.filter(title=subject,type=type, group=group)])
        return activities
    else:
        activities = []
        for group in Specialization.groups[spec]:
            activities.extend(
                [FacultyActivity(activity) for activity in SchoolActivity.objects.filter(professor=prof,subject=subject,type=type, group=group)])
        return activities
